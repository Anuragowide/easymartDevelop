...
<html class="no-js no-touch" lang="{{ request.locale.iso_code }}">
  ...
  >
        ...
        <div class="atc-banner--outer">
          <div class="atc-banner--inner">
            <div class="atc-banner--product">
              <h2 class="atc-banner--product-title">
                <span class="atc-banner--product-title--icon">
                  {% render 'icon-library', id: 'icon-checkmark' %}
                </span>
            ...
            <div class="atc-banner--cart">
              ...
              {%- render 'free-shipping-bar', context: 'atc-banner' -%}
            ...
            <template data-product-compare-drawer-item-template>
              ...
              >
                ...
                <button class="product-compare-drawer__remove" data-product-compare-drawer-remove>
                  {% render 'icon-library', id: 'icon-remove', icon_class: 'product-compare-drawer__remove-icon' %}
                </button>
              </div>
            </template>
   ...
   <!-- ============================= -->
<!-- Easymart AI Chatbot START -->
<!-- ============================= -->

<script>
  window.EASYMART_BACKEND_URL = "https://node.gowideai.com/api/chat";
</script>
<script src="https://node.gowideai.com/widget/chat-widget.js" defer></script>
<link rel="stylesheet" href="https://node.gowideai.com/widget/chat-widget.css" />

<script>
// Override widget to handle search_results action
  ...
  setTimeout(function() {
    ...
    window.fetch = function(url, options) {
      return originalFetch.apply(this, arguments).then(function(response) {
        // Only intercept our chat API calls
        if (url && url.includes('/api/chat')) {
          return response.clone().json().then(function(data) {
            // Handle search_results action - render products
            if (data.actions && Array.isArray(data.actions)) {
              setTimeout(function() {
                ...
                data.actions.forEach(function(action) {
                  if (action.type === "search_results" && action.data && action.data.results) {
                    action.data.results.forEach(function(product) {
